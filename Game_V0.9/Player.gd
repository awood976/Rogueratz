	...
	if is_in_dash:
		...
		if controller_connected:
			...
			if abs(rs_x) > controller_deadzone:
				rs_position.x = rs_x*get_viewport().size.x/0.5*delta
			if abs(rs_y) > controller_deadzone:
				rs_position.y = rs_y*get_viewport().size.y/0.5*delta
			Input.warp_mouse_position(get_viewport().get_mouse_position()+rs_position)	
		
		
		var xAxis = Input.get_joy_axis(controller_id, JOY_AXIS_0)
		...
		if Input.is_action_just_pressed("dash") and Input.is_action_pressed("ui_left") and not has_dashed:
			has_dashed = true
			dash_horizontal = -1
			dash_vertical = 0
			start_timer()
		if Input.is_action_just_pressed("dash") and Input.is_action_pressed("ui_right") and not has_dashed:
			has_dashed = true
			dash_horizontal = 1
			dash_vertical = 0
			start_timer()
		if Input.is_action_just_pressed("dash") and Input.is_action_pressed("ui_up") and not has_dashed:
			has_dashed = true
			dash_horizontal = 0
			dash_vertical = -1
			start_timer()
		if Input.is_action_just_pressed("dash") and Input.is_action_pressed("ui_down") and not has_dashed:
			has_dashed = true
			dash_horizontal = 0
			dash_vertical = 1
			start_timer()

...
#			$"Gun Sprite/Flame/Particles2D".emitting = false
		...
		var rs_x = Input.get_joy_axis(0, JOY_AXIS_2)
		var rs_y = Input.get_joy_axis(0, JOY_AXIS_3)
		
		if abs(rs_x) > controller_deadzone:
			rs_position.x = rs_x*get_viewport().size.x/1.5*delta
		if abs(rs_y) > controller_deadzone:
			rs_position.y = rs_y*get_viewport().size.y/1.5*delta
